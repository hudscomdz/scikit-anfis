...
class GaussMembFunc(torch.nn.Module):
    ...
    def __init__(self, mu, sigma):
        ...
        self.mu.register_hook(GaussMembFunc.mu_log_hook)
        self.sigma.register_hook(GaussMembFunc.sigma_log_hook)

    @staticmethod
    def mu_log_hook(grad):
        '''
...
class TriangularMembFunc(torch.nn.Module):
    ...
    def forward(self, x):
        return torch.where(
            torch.Tensor(self.a < x) & torch.Tensor(x < self.b),
            (x - self.a) * (1/(self.b - self.a)) if self.a != self.b else 1.0,
            # else
            torch.where(
                torch.Tensor(self.b <= x) & torch.Tensor(x <= self.c),
                (self.c - x) / (self.c - self.b) if self.c != self.b else 1.0,
...
def make_anfis(x, num_mfs=5, num_out=1, hybrid=True):
    ...
    '''
    from .scikit_anfis import scikit_anfis
    ...
    model = scikit_anfis('Simple classifier', invars, outvars, hybrid=hybrid)
    return model
